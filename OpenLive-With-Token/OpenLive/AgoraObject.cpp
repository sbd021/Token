...
#include <stdio.h>
std::string app_id = "";
CAgoraConfig CAgoraConfig::config;

CAgoraConfig* CAgoraConfig::GetAgoraConfig()
{
	return &config;
}

void CAgoraConfig::SetAppid(CString id)
{
	char szAppid[128] = { 0 };
	::WideCharToMultiByte(CP_UTF8, 0, id.GetBuffer(), -1, szAppid, 128, NULL, NULL);

	appid = szAppid;
}
void CAgoraConfig::SetAppCerf(CString cerf)
{
	char szAppCerf[1024] = { 0 };
	::WideCharToMultiByte(CP_UTF8, 0, cerf.GetBuffer(), -1, szAppCerf, 1024, NULL, NULL);
	appCerf = szAppCerf;
}
...
#endif
	app_id = CAgoraConfig::GetAgoraConfig()->GetAppid();
