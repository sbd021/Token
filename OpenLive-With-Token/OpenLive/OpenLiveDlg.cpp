...
LRESULT COpenLiveDlg::OnJoinChannel(WPARAM wParam, LPARAM lParam)
{
	...
	std::string appid = CAgoraConfig::GetAgoraConfig()->GetAppid();
	std::string appcerf = CAgoraConfig::GetAgoraConfig()->GetAppCerf();

	char szChannel[128] = { 0 };
	::WideCharToMultiByte(CP_UTF8, 0, strChannelName.GetBuffer(), -1, szChannel, 128, NULL, NULL);

	unsigned int expirationTimeInSeconds = 3600;
	unsigned int currentTimeStamp = time(NULL);
	unsigned int privilegeExpiredTs = currentTimeStamp + expirationTimeInSeconds;

	const char* token = generateAgoraRtcTokenWithUid(appid.c_str(), appcerf.c_str(), szChannel, 0, Role_Publisher, 0);
	
	lpAgoraObject->JoinChannel(strChannelName, 0,/*strlen(APP_TOKEN) > 0 ? APP_TOKEN:NULL*/token);
